...
namespace esphome
{
    namespace panaac
    {
        ...
        void PanaACFanLevel::control(const std::string &value)
        {
            ...
            // Deactivate POWERFUL when fan level is set
            if (this->climate_->ac_state.preset == climate::CLIMATE_PRESET_BOOST)
            {
                this->climate_->ac_state.preset = climate::CLIMATE_PRESET_NONE;
                if (this->climate_->preset_ != nullptr)
                    this->climate_->preset_->set_preset(climate::CLIMATE_PRESET_NONE);
            }
